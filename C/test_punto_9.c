...
void mediaFile(char* nomeFile1, char* nomeFile2){
	FILE *pFileA;
	FILE *pFileB;
	veicolo autoA;
	veicolo autoB;
	...
	if((pFileA!=NULL)&&(pFileB!=NULL)){
		...
		printf("La media dei prezzi presenti nei file %s e %s e\' pari a: %0.2f\n", nomeFile1, nomeFile2, media);                          //%0.2f significa due cifre significative dopo lo zero
		fclose(pFileA);
		fclose(pFileB);	
	}
	else{
...
void minMax(char* nomeFile1, char* nomeFile2){
	FILE *pFileA;
	FILE *pFileB;
    veicolo autoA;
    veicolo autoB;
	...
	if((pFileA!=NULL)&&(pFileB!=NULL)){
		...
		printf("Il minimo contenuto nei file %s e %s e\': %d",nomeFile1, nomeFile2, min);
		printf("\nIl massimo contenuto nei file %s e %s e\': %d",nomeFile1,nomeFile2, max);
		fclose(pFileA);
		fclose(pFileB);	
	}
	else{
